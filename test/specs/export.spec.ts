...
describe("partition export", () => {
  test("exports a single partition to tar.gz with manifest metadata and preserved bytes", async () => {
    ...
    writePartitionLogFile(dir, "alpha", "jobs.queue", "2026-05-17-10-0000-info.jsonl", [{
    ...
    writePartitionLogFile(dir, "alpha", "jobs.queue", "2026-05-17-11-0000-warn.jsonl", [{
    ...
    writePartitionLogFile(dir, "beta", "ops.audit", "2026-05-17-10-0000-info.jsonl", [{
      recorded_at: "2026-05-17T10:00:00.000Z",
      level: "info",
      group: "ops.audit",
      message: "beta-info",
    ...
    expect(entries.has(`${root}/logs/alpha/jobs/queue/2026-05-17-10-0000-info.jsonl`)).toBe(true);
    expect(entries.has(`${root}/logs/alpha/jobs/queue/2026-05-17-11-0000-warn.jsonl.gz`)).toBe(true);
    expect(entries.has(`${root}/logs/beta/ops/audit/2026-05-17-10-0000-info.jsonl`)).toBe(false);
    expect(manifest.partitions).toEqual(["alpha"]);
    expect(manifest.total.files).toBe(2);
    expect(manifest.total.logs).toBe(2);

    const sourceGzip = fs.readFileSync(path.join(dir, "alpha", "jobs", "queue", "2026-05-17-11-0000-warn.jsonl.gz"));
    expect(entries.get(`${root}/logs/alpha/jobs/queue/2026-05-17-11-0000-warn.jsonl.gz`)).toEqual(sourceGzip);
  });
  ...
  test("exports multiple partitions to zip with raw files for each selected partition", async () => {
    ...
    writePartitionLogFile(dir, "beta", "ops.audit", "2026-05-17-10-0000-error.jsonl", [{
    ...
    expect(entries.has(`${root}/logs/alpha/jobs/queue/2026-05-17-10-0000-info.jsonl`)).toBe(true);
    expect(entries.has(`${root}/logs/beta/ops/audit/2026-05-17-10-0000-error.jsonl`)).toBe(true);
  });
